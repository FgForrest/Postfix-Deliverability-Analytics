...
/**
 * A convenient singleton for estimating how much memory do objects occupy.
 * It is checking for CompressedOops JVM option, UseCompressedStrings just makes JVM use byte[] instead of char[]
 * which is also covered.
 */
object MemoryFootprintAnalyzer {
  val log = LoggerFactory.getLogger("MemoryFootprintAnalyzer")
    ...
    if (elementClass.isPrimitive) {
      ...
      if (length <= ARRAY_SIZE_FOR_SAMPLING) {
        ...
        for (i <- 0 until ARRAY_SAMPLE_SIZE) {
          ...
          size += MemoryFootprintAnalyzer.estimate(elem, state.visited)
        }
